...
(async () => {
  ...
  try {
    ...
    for (const file of pages) {
      ...
      assert.equal(await page.locator('.nav-link--dropdown').getAttribute('aria-expanded'), 'true');
    ...
    await page.locator('.nav-link--dropdown').focus(); await page.keyboard.press('Enter'); await page.keyboard.press('Tab');
    ...
    console.log('All layouts, links, menus, keyboard, fallback and accessibility checks passed.');
  } finally {
    fs.writeFileSync(path.join(out, 'results.json'), JSON.stringify(results, null, 2));
