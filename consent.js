...
(() => {
  ...
  banner.innerHTML = `<div><h2>Jouw cookiekeuze</h2><p>Met jouw toestemming gebruiken we Google Analytics om te zien welke pagina’s worden bekeken. Zonder toestemming blijft Analytics uit. Je kunt je keuze altijd wijzigen via Cookie-instellingen onderaan de pagina.</p><p class="cookie-pending" ${configured ? 'hidden' : ''}>Google Analytics is nog niet ingesteld. Zodra het wordt aangezet, vragen we je opnieuw om een keuze.</p><a href="privacyverklaring.html#artikel-8">Lees ons cookiebeleid</a></div><div class="cookie-actions"><button type="button" data-choice="accept">Accepteren</button><button type="button" data-choice="reject">Weigeren</button><button type="button" data-choice="settings">Instellingen</button></div>`;
  const dialog = document.createElement('dialog');
  dialog.className = 'cookie-dialog';
  dialog.setAttribute('aria-labelledby', 'cookie-dialog-title');
  dialog.innerHTML = `<h2 id="cookie-dialog-title">Cookie-instellingen</h2><p>We bewaren je keuze maximaal 180 dagen op dit apparaat. Hiervoor gebruiken we lokale opslag.</p><p><strong>Noodzakelijk</strong><br>Je privacykeuze onthouden. Altijd actief.</p><label class="cookie-option"><input type="checkbox" id="cookie-analytics"> <span><strong>Bezoekersstatistieken</strong><br>Google Analytics helpt ons begrijpen hoe de website wordt gebruikt. Alleen met jouw toestemming.</span></label><p class="cookie-pending" ${configured ? 'hidden' : ''}>Analytics is nog niet ingesteld. Je keuze activeert nu geen metingen.</p><div class="cookie-actions"><button type="button" data-choice="save">Keuze opslaan</button><button type="button" data-choice="reject">Alles weigeren</button><button type="button" data-choice="close">Sluiten</button></div>`;
